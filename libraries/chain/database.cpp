...
void database::open( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t initial_supply, uint64_t shared_file_size, uint32_t chainbase_flags )
{
   try
   {
      ...
      {
         if( auto dgpo = find< dynamic_global_property_object >() )
            modify( *dgpo, [&]( dynamic_global_property_object &o )
            {
                o.time = STEEMIT_GENESIS_TIME;
            });
         else
            with_write_lock( [&]()
            {
   ...
   )
{
   ...
   if( has_hardfork( STEEMIT_HARDFORK_0_5__54 ) )
   {
      ...
      << fc::string(witness.running_version) << " ,"
      << fc::string(STEEMIT_BLOCKCHAIN_VERSION) << ", "
      << fc::string(hfp.current_hardfork_version) << ", "
      << fc::string(STEEMIT_BLOCKCHAIN_HARDFORK_VERSION) << ", "
      << fc::string(witness.hardfork_version_vote) << ", "
      << fc::string(_hardfork_versions[ hfp.last_hardfork + 1 ]) << ", "
      << fc::string(witness.hardfork_time_vote) << ", "
      << fc::string(_hardfork_times[ hfp.last_hardfork + 1 ]) << '\n';
...
void create_or_modify( database& db, const account_name_type& account_name, Modifier&& m )
{
    ...
    else
    {
        ...
        op.new_account_name = account_name;
        operation_notification note(op);
        db.notify_pre_apply_operation( note );
    ...
    with_write_lock( [&]()
    {
       try
       {
          ...
          for( const auto& account : snapshot.accounts )
          {
             ...
             {
                a.name = account.name;
                a.proxy = account.proxy;
                a.post_count = account.post_count;
                a.memo_key = account.keys.memo_key;
                const auto &metadata = account.json_metadata;
                a.json_metadata.assign(metadata.begin(), metadata.end());
          ...
          modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
          {
              ...
              gpo.current_supply = summary.balance;
              gpo.virtual_supply = gpo.current_supply;
              gpo.current_sbd_supply = summary.sbd_balance;
              gpo.total_vesting_shares = summary.total_vesting_shares;
              gpo.total_vesting_fund_steem = summary.total_vesting_fund_steem;
          } );
          validate_invariants();
       }
       FC_CAPTURE_AND_RETHROW();
   ...
   while( itr != next_block.extensions.end() )
   {
      switch( itr->which() )
      {
         ...
         case 2: // hardfork_version vote
         {
            ...
            std::cout << __PRETTY_FUNCTION__
            << fc::string( hfv.hf_version) << ", "
            << fc::string( signing_witness.hardfork_version_vote) << ", "
            << hfv.hf_time.sec_since_epoch() << ", "
            << signing_witness.hardfork_time_vote.sec_since_epoch() << '\n';
...
void database::process_hardforks()
{
   try
   {
      ...
      if( has_hardfork( STEEMIT_HARDFORK_0_5__54 ) )
      {
          ...
          << fc::string(_hardfork_versions[ hardforks.last_hardfork ]) << ", "
          <<  fc::string(hardforks.next_hardfork) << ", "
          << hardforks.next_hardfork_time.sec_since_epoch() << ", "
      ...
      else
      {
          ...
          << hardforks.last_hardfork+1 << ", "
          << _hardfork_times[ hardforks.last_hardfork + 1 ].sec_since_epoch() << ", "
          << head_block_time().sec_since_epoch() << "\n";
