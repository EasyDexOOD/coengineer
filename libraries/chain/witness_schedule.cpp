...
void update_witness_schedule4( database& db )
{
   ...
   << " widx.size(): " << widx.size()
   << " max voted witness: " << int(wso.max_voted_witnesses) << "\n";
   ...
   std::cout << __PRETTY_FUNCTION__
   << " pow_idx.size(): " << pow_idx.size()
   << " max miner witness: " << int(wso.max_miner_witnesses) << "\n";
   ...
   std::cout << __PRETTY_FUNCTION__
   << " schedule_idx.size(): " << schedule_idx.size()
   << " new_virtual_time: " << std::string(new_virtual_time) << "\n";
   ...
   if( db.has_hardfork( STEEMIT_HARDFORK_0_5__54 ) )
   {
      ...
      std::cout << __PRETTY_FUNCTION__
      << " num_scheduled_witnesses: " << int(wso.num_scheduled_witnesses) << '\n';
      ...
      while( hf_itr != hardfork_version_votes.end() )
      {
         if( hf_itr->second >= wso.hardfork_required_witnesses )
         {
            ...
            if( hfp.next_hardfork != std::get<0>( hf_itr->first ) ||
                ...
                std::cout << __PRETTY_FUNCTION__ << " : " << std::get<1>( hf_itr->first ).sec_since_epoch() << '\n';
